---
name: jgl-scope
description: Guided-start interview for a new project — a 15-20 minute conversation about what you're building, for whom, goals, constraints, brand, and stack that produces docs/SCOPE.md (and docs/BRAND.md when there's a brand), adds a Project context section to CLAUDE.md, and picks the first milestone. Use when a project is new/empty, when the user says "help me get started", "scope this out", or when work keeps stalling because the goal was never written down. If the kit ships a domain-specific scoping skill, run that instead — it knows the domain's questions.
---
