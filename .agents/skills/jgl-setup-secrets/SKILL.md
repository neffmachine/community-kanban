---
name: jgl-setup-secrets
description: Set up encrypted-in-repo secrets management (SOPS + age) with separate dev/prod keys, verified key rotation, and a pre-commit guard. Use when the user says "set up secrets", "/jgl-setup-secrets", "encrypt my secrets", "manage API keys", or asks how to handle secrets/.env safely.
---
